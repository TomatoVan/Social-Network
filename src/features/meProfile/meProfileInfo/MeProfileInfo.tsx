...
export const MeProfileInfo: FC<ProfileInfoType> = memo(({ profile, status, setEditCallback }) => {
    ...
    <div className={s.profileInform}>
      <div className={s.profileInfoLeft}>
        <div className={s.container}>
          ...
          <button className={s.editProfileButton} onClick={changeEditModeProfileCallback}>
            Edit profile
          </button>
        </div>
              ...
              if (contact[1]) {
                return (
                  <div key={contact[0]}>
                    ...
                    : <a href={`http://${contact[1]}`}>{contact[1]}</a>
                  </div>
                )
              } else return null
            })}
