...
//AC
export const setFollowing = (userId: number) =>
  ({ type: 'CHANGE-FOLLOWING', payload: { userId } } as const)
...
export const setFollow =
  (userId: number, isFollow: boolean): AppThunkType =>
  async dispatch => {
    try {
      let response
