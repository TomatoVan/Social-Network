...
//reducer
export const meProfileReducer = (
  state: MeProfileStateType = initialState,
  action: MeProfileActionsType
): MeProfileStateType => {
  switch (action.type) {
    case 'ME/ADD-POST':
      return {
        ...
        postsData: [
          ...state.postsData,
          { id: Number(v1()), message: action.payload.newPost, likes: 0, comments: 0, shares: 0 },
        ],
      }
...
export const addMyPost = (newPost: string) =>
  ({ type: 'ME/ADD-POST', payload: { newPost } } as const)
export const setMyProfile = (profile: ProfileType) =>
  ({ type: 'ME/SET-USER-PROFILE', payload: { profile } } as const)
export const setMyStatus = (status: string) =>
  ({ type: 'ME/SET-USER-STATUS', payload: { status } } as const)
export const saveMyPhotoSuccess = (photos: PhotosType) =>
  ({ type: 'ME/SAVE-PHOTOS-SUCCESS', payload: { photos } } as const)
...
export const updateMyProfile =
  (userId: string, profileData: ProfileType): AppThunkType =>
  async dispatch => {
    dispatch(changeAppStatus('loading'))
    try {
...
export type MeProfileActionsType =
  | AddPostType
  | SetUserProfileType
  | SetUserStatusType
